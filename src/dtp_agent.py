...
import gc
from constr_agent import ConstrLinAgent
from abstract_agent import Verbosity


class DTPAgent(ConstrLinAgent):
    ...
    def file_based_dtproblog(self, model):
        ...
        if self.verbose >= Verbosity.debug:
            print("{} is calculating dtp model: {}".format(
    ...
    def format_utility_string(self):
        ...
        for offer, score in self.generated_offers:
            ...
            return_string += "offer{} :- {}.\n".format(
                offer_counter, ",".join(atom_list))
            return_string += "utility(offer{},{}).\n".format(
                offer_counter, -score + self.non_agreement_cost)
            offer_counter += 1

        for constr in self.get_all_constraints():
            return_string += "utility({}_{},{}).\n".format(
                constr.issue, constr.value, -(2 ** 31))
    ...
    def generate_offer(self):
        ...
        gc.collect()
        if self.verbose >= Verbosity.debug:
            print("{} generated offer: {}".format(self.agent_name,
                                                  self.nested_dict_from_atom_dict(query_output)))

        # if there are no acceptable offers left we should terminate
        if score < self.absolute_reservation_value:
            return None
